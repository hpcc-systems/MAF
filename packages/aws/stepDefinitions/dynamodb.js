...
function tableExists(tableName) {
  var res = runAWS("dynamodb list-tables");
...
MAFWhen("{jsonObject} is cleaned", function (payload) {
  ...
  var cleanedItem = cleanDynamoQuery(payload);
  return cleanedItem;
});
...
MAFWhen("{jsonObject} is converted to dynamo", function (payload) {
  var payload = JSON.stringify(performJSONObjectTransform.call(this, payload));
  var dynamoItem = convertToDynamoItem(payload);
  if (!this.results) {
    this.results = {};
  }
  return dynamoItem;
...
When("dynamodb query from {jsonObject} is performed", function (payload) {
  var payload = performJSONObjectTransform.call(this, payload);
...
When("perform dynamodb query:", function (docString) {
  ...
  var payload = JSON.parse(fillTemplate(docString, this.results));
  performDynamoDBQueryFromJSON.call(this, payload);
});
