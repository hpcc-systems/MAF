...
const { performJSONObjectTransform, filltemplate } = require('@ln-maf/core')
...
const bddSendMessageToQueue = function (message, queue) {
  const res = sendMessageToQueue.call(this, message, queue)
  if (!this.results) {
    this.results = {}
  }
...
When('the next message is received from queue {string}', function (queueName) {
  queueName = getVal(queueName, this)
  const res = dequeueMessagesFromQueue(queueName)
  if (!this.results) {
    this.results = {}
  }
...
When('{int} messages are received from queue {string}', function (numOfMessages, queueName) {
  queueName = getVal(queueName, this)
  const res = dequeueMessagesFromQueue(queueName, numOfMessages)
  if (!this.results) {
    this.results = {}
  }
...
When('attributes of queue {string} are received', function (queueName) {
  queueName = getVal(queueName, this)
  const res = getQueueAttributes(queueName)
  if (!this.results) {
    this.results = {}
  }
  this.results.lastRun = res
